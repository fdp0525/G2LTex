...
class TexturePatch
{
    public:
        ...
        typedef std::vector<std::size_t> Vertices;
        typedef std::vector<math::Vec2f> Texcoords;

    private:
        int label;//该patch的索引
        Faces faces;//该patch下所有面的索引
        Vertices  vertices;
        Texcoords texcoords;//所面中所有顶点对应的纹理为坐标
        mve::FloatImage::Ptr image;//纹理对应的子图像。
    ...
    public:
        ...
        TexturePatch(int _label, std::vector<std::size_t> const & _faces, std::vector<std::size_t> const & _vertices,
            std::vector<math::Vec2f>  const & _texcoords, mve::FloatImage::Ptr _image);
...
//        {std::cout<<"bbbbbbbbbbbbb"<<std::endl;}
        ...
        static TexturePatch::Ptr create(int label, std::vector<std::size_t> const & faces,
                                        std::vector<std::size_t> const & vertices,
            std::vector<math::Vec2f> const & texcoords, mve::FloatImage::Ptr image);
        ...
        std::vector<std::size_t> & get_vertices(void);
        std::vector<std::size_t> const & get_vertices(void) const;
...
TexturePatch::create(int label, std::vector<std::size_t> const & faces,
    ...
    std::vector<math::Vec2f>  const & texcoords, mve::FloatImage::Ptr image)
{
    return std::make_shared<TexturePatch>(label, faces, vertices, texcoords, image);
}
...
inline std::vector<std::size_t> &
TexturePatch::get_vertices(void) {
    return vertices;
}
...
inline std::vector<std::size_t> const &
TexturePatch::get_vertices(void) const {
    return vertices;
}
