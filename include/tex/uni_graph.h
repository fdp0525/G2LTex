...
class UniGraph {
    private:
        std::vector<std::vector<std::size_t> > adj_lists;//每个面所有的邻接面
    ...
    public:
        ...
        //add by fuyp 2017-09-14
        void get_alone_subGraphs(std::size_t label, std::vector<std::vector<std::size_t> > * subgraphs) const;

        std::vector<std::size_t> const & get_adj_nodes(std::size_t node) const;
};

/**
 * @brief UniGraph::add_edge  建立n1，n2之间的邻接图（）
 * @param n1
 * @param n2
 */
...
{
    assert(n1 < num_nodes() && n2 < num_nodes());
    std::vector<std::size_t> const & adj_list = adj_lists[n1];//n1面的邻接面片
