...
class SparseTable {
    public:
        typedef std::vector<std::pair<R, T> > Column;//每个面对应的所有视口信息<R对应的是视口索引，T是cost值>
        typedef std::vector<std::pair<C, T> > Row;//每个视口对应的所有面的信息<C对应的是面的索引，T是cost值>

    private:
        std::vector<Column> column_wise_data;//每个面对应的所有视口信息
        std::vector<Row> row_wise_data;//每个视口对应的每个面的信息
    ...
    public:
        ...
        C cols() const;//返回column_wise_data的大小
        R rows() const;//返回row_wise_data的大小
