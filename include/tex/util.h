...
multi_gauss_unnormalized(Eigen::Matrix<T, 1, N> const & X, Eigen::Matrix<T, 1, N> const & mu,
    ...
    return std::exp(T(-0.5) * mean_removed * covariance_inv * mean_removed.adjoint());//adjoint伴随矩阵
}
...
/**
  ...
  * @throws util::FileException
  */
template <typename T> void vector_to_file(std::string const & filename, std::vector<T> const & vector)
{
    std::ofstream out(filename.c_str(), std::ios::binary);
//    std::ofstream out(filename.c_str());//不要存成二进制
    if (!out.good())
        throw util::FileException(filename, std::strerror(errno));
...
/**
 * @brief writeLabelToFile  把每个面的标签写入文件
 * @param labels
 */
inline void writeLabelToFile(std::string filename, std::vector<std::size_t> &labels)
{
    std::ofstream out(filename.c_str());
