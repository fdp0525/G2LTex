...
/* ---------------------------------------------------------------- */
/**
 * @brief Arguments::add_option  参数加入可选参数序列夹
 * @param shortname    短名（缩写）
 * @param longname     完整名（全名）
 * @param has_argument  是否需要参数
 * @param description       描述
 */
void Arguments::add_option (char shortname, std::string const& longname, bool has_argument, std::string const& description)
{
    ...
    {
        if ((shortname != '\0' && this->options[i].sopt == shortname) || (!longname.empty() && this->options[i].lopt == longname))
        {
    ...
    this->options.push_back(opt);//加入可选参数序列
}

/* ---------------------------------------------------------------- */
/**
 * @brief Arguments::parse  解析输入参数
 * @param argc
 * @param argv
 */
void Arguments::parse (int argc, char const* const* argv) throw(util::Exception)
{
...
void Arguments::parse (std::vector<std::string> const& args)  throw(util::Exception)
{
    try
    {
    ...
    catch (util::Exception const& e)
    {
        ...
        std::cerr << std::endl;
//        this->generate_helptext(std::cerr);//输出所有的帮助信息
...
/* ---------------------------------------------------------------- */
/**
 * @brief Arguments::parse_intern   对输入参数进行逐项解析
 * @param args
 */
void Arguments::parse_intern (std::vector<std::string> const& args)
{
    ...
    for (std::size_t i = 0; i < args.size(); ++i)
    {
        ...
        string::clip_newlines(&tok);//去掉换行符
        string::clip_whitespaces(&tok);//去掉空白
