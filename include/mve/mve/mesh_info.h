...
public:
    ...
    struct VertexInfo
    {
        VertexClass vclass;//点的类型
        AdjacentVertices verts;//每个点的所有三角形的邻接点邻接点，按OpenGL绘制扇形三角形存储顶点索引。
        AdjacentFaces faces;//每个点的所有邻接面
...
private:
    std::vector<VertexInfo> vertex_info;//每个顶点包含的信息（包括该点的类型，他的邻接面和邻接顶点）
};
