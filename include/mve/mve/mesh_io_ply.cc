...
TriangleMesh::Ptr load_ply_mesh (std::string const& filename)
{
    ...
    TriangleMesh::VertexList& vertices = mesh->get_vertices();//mesh顶点
    TriangleMesh::FaceList& faces = mesh->get_faces();//面（保存面的三个顶点）
    TriangleMesh::ColorList& vcolors = mesh->get_vertex_colors();//顶点颜色
    TriangleMesh::ConfidenceList& vconfs = mesh->get_vertex_confidences();
    TriangleMesh::ValueList& vvalues = mesh->get_vertex_values();
    TriangleMesh::TexCoordList& tcoords = mesh->get_vertex_texcoords();//顶点纹理坐标
    TriangleMesh::NormalList& vnormals = mesh->get_vertex_normals();//点法线
    ...
    faces.reserve(num_faces * 3);//保存face的三个顶点
